...
public class SnapScrolling : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int i = 0; i < panCount; i++)
        {
            instPans[i] = transform.GetChild(i).gameObject;
            pansPos[i] = instPans[i].transform.localPosition;
           // if (i == 0) continue;
            /*
            instPans[i].transform.localPosition = new Vector2(instPans[i - 1].transform.localPosition.x + panPrefab.GetComponent<RectTransform>().sizeDelta.x + panOffset,
    ...
    private void FixedUpdate()
    {
        ...
        {
            float distance = 
                Mathf.Abs(contentRect.anchoredPosition.x 
                - pansPos[i].x);
            Debug.Log(i + " D : " + distance);
        ...
        contentVector.x = Mathf.SmoothStep(contentRect.anchoredPosition.x, pansPos[selectedPanID].x + SnapPointOffset, snapSpeed * Time.fixedDeltaTime);
